...
export default function IntroAnimationOverlay({ onEnd }: Props) {
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      if (typeof console !== 'undefined') console.log('[FleetPulse] intro failsafe: 4s timeout')
  ...
  return (
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 60% 60% at 50% 50%, rgba(139, 92, 246, 0.2) 0%, transparent 60%)' }} />
      ...
      >
        <div className="w-full max-w-[90vw] md:max-w-[520px] rounded-xl shadow-xl flex items-center justify-center" style={{ filter: 'drop-shadow(0 0 40px rgba(139, 92, 246, 0.25))' }}>
          <video
            ref={videoRef}
            autoPlay
            muted
