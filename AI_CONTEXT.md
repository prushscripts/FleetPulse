...
8. **Safe changes / regressions:** Before changing root layout (`app/layout.tsx`), scroll-dependent components (ScrollBlur, FloatingLoginCard, ScrollToTop), error boundaries, or intro/first-load: read **`docs/SAFE_CHANGES_GUARDRAILS.md`** and run the checklist there. Keep the document/body as the only scroll container; use local error boundaries for section failures. Known-good reference: `Restore Points/FleetPulseRestoreReference/`.
